...
from franken.utils.linalg.bayesiansolve import bayesian_linear
    ...
    def fit_bayesian(
        ...
        Args:
            ...
            solver_params (dict): Parameters for the solver which actually
                ...
                will result in four different models, one with :code:`alpha=1e-4, beta=1e-4, force_weight=0.5`
                one with :code:`alpha=1e-4, beta=1e-6, force_weight=0.5`, one with :code:`alpha=1e-6, beta=1e-4, force_weight=0.5`
                and one with :code:`alpha=1e-6, beta=1e-6, force_weight=0.5`. This way of specifying solver
    ...
    def evaluate_bayesian(
        ...
        for i, (data, targets) in enumerate(progress_bar):
            ...
            if split == DataSplit.TRAIN and self.save_fmaps:
                ...
                if all_weights_mean is None or all_weights_mean.shape[0] <= 100:
